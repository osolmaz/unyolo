...
func TestRenderLaunchd(t *testing.T) {
	body, err := RenderLaunchd(LaunchdUnit{
		Label: "dev.brokerkit.huggingface", ProgramArguments: []string{"/usr/local/bin/hf-broker", "serve"},
		...
		Sockets: []LaunchdSocket{
			{Name: "agent", Path: "/var/run/brokerkit/huggingface/agent/broker.sock", Owner: "root", Group: "hf-broker-agent", Mode: 0o660, DirectoryMode: 0o750},
			{Name: "operator", Path: "/var/run/brokerkit/huggingface/operator/broker.sock", Owner: "root", Group: "hf-broker-operator", Mode: 0o660, DirectoryMode: 0o750},
		},
	})
	if err != nil {
		t.Fatal(err)
	...
	for _, want := range []string{"<string>dev.brokerkit.huggingface</string>", "<string>activation://agent</string>", "<key>agent</key>", "<integer>432</integer>", "<integer>5</integer>"} {
...
func TestRenderLaunchdEscapesValues(t *testing.T) {
	body, err := RenderLaunchd(LaunchdUnit{
		Label: "dev.brokerkit.test", ProgramArguments: []string{"/usr/local/bin/test", "a&b"}, UserName: "broker", GroupName: "broker",
...
func TestRenderLaunchdRejectsUnsafeValues(t *testing.T) {
	base := LaunchdUnit{Label: "dev.brokerkit.test", ProgramArguments: []string{"/usr/local/bin/test"}, UserName: "broker", GroupName: "broker", Sockets: []LaunchdSocket{{Name: "agent", Path: "/var/run/test.sock", Owner: "root", Group: "broker", Mode: 0o660, DirectoryMode: 0o750}}}
