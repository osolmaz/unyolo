...
install_binary() {
  ...
  if [ -w "$dest_dir" ]; then
    install -m 0755 "$source_path" "$dest_dir/hf-broker"
    return
  fi
  if command -v sudo >/dev/null 2>&1; then
    sudo install -d -m 0755 "$dest_dir"
    sudo install -m 0755 "$source_path" "$dest_dir/hf-broker"
    return
  fi
...
asset="hf-broker_${os}_${arch}.tar.gz"
...
install_binary "${tmp_dir}/hf-broker" "$dest_dir"

echo "Installed hf-broker to ${dest_dir}/hf-broker"
if command -v hf-broker >/dev/null 2>&1; then
  hf-broker --version
elif [ -x "${dest_dir}/hf-broker" ]; then
  "${dest_dir}/hf-broker" --version
  echo "Add ${dest_dir} to PATH to run hf-broker directly."
fi
