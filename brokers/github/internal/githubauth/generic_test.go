...
func TestGitHubErrorsPreserveSanitizedProviderDiagnostics(t *testing.T) {
	graphResponse := &http.Response{StatusCode: http.StatusOK, Header: http.Header{"X-Github-Request-Id": []string{"ABCD:1234"}},
		Body: io.NopCloser(strings.NewReader(`{"errors":[{"type":"UNPROCESSABLE","message":"Merge commits are not allowed on this repository."}]}`))}
	_, err := decodeGraphQLResponse(graphResponse, graphqlmanifest.Document{})
	var graphError APIError
	if !errors.As(err, &graphError) || graphError.Code != "unprocessable" || graphError.Message != "Merge commits are not allowed on this repository." || graphError.RequestID != "ABCD:1234" {
	...
	restResponse := &http.Response{StatusCode: http.StatusUnprocessableEntity, Header: http.Header{"X-Github-Request-Id": []string{"REST-123"}},
		Body: io.NopCloser(strings.NewReader(`{"message":"Validation Failed","errors":[{"code":"custom","message":"Repository rule blocked this update"},"A second validation detail"]}`))}
	var restError APIError
	if err := classifyHTTPError(restResponse); !errors.As(err, &restError) || restError.Message != "Validation Failed: Repository rule blocked this update: A second validation detail" || restError.RequestID != "REST-123" {
		t.Fatalf("REST error = %+v, %v", restError, err)
	}

	if safeGitHubMessage("A new provider diagnostic that was not seen during development") != "A new provider diagnostic that was not seen during development" {
		t.Fatal("previously unseen provider diagnostic was discarded")
	}
	for _, unsafe := range []string{"bad request id", "gh" + "p_" + "ABCDEFGHIJKLMNOPQRSTUVWXYZ123456", "github_" + "pat_" + "ABCDEFGHIJKLMNOPQRSTUVWXYZ123456"} {
		if githubRequestID(http.Header{"X-Github-Request-Id": []string{unsafe}}) != "" {
			t.Fatalf("unsafe GitHub request ID was retained: %q", unsafe)
		}
	}
	for _, unsafe := range []string{"NOT SAFE!", "gh" + "p_" + "abcdefghijklmnopqrstuvwxyz123456", "unreviewed_provider_code"} {
