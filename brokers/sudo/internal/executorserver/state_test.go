...
func TestExecutionStateClaimsPersistsAndReplays(t *testing.T) {
	...
	record, claimed, err := state.claim("execution-1", "digest-1")
	...
	replay, claimed, err := restarted.claim(record.ID, "digest-1")
	...
	if _, _, err := restarted.claim(record.ID, "other"); !errors.Is(err, errExecutionConflict) {
...
func TestExecutionStateLeavesInterruptedClaimsAmbiguous(t *testing.T) {
	...
	_, _, _ = state.claim("execution-1", "digest-1")
	record, claimed, err := state.claim("execution-1", "digest-1")
	if err != nil || claimed || record.Status != executionClaimed {
