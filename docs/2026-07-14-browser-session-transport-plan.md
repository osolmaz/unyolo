...
that popover. unYOLO must not replace those controls with a **Review
...
unYOLO browser UI clients authenticate every protected UI API request with
...
`unyolo-session` contains the raw opaque browser session or direct UI
...
Server-to-server unYOLO Operator V1 clients continue to authenticate their
operator API requests with `Authorization: Bearer <operator-credential>`.

This is an in-place replacement of the pre-release
`unyolo.io/delegated-web/v1` transport rule. Version 1 is the only supported
...
Make every unYOLO browser UI mode portable behind hosting layers that
...
outer OpenClaw page, unYOLO iframe document, and packaged JavaScript loaded,
but each request to the delegated snapshot endpoint returned `404` before it
reached MLClaw. The browser resource record was:

```text
/mlclaw/api/unyolo/snapshot  404
```

The unYOLO UI currently sends its short-lived delegated token as
...
- translating safe delegated calls to unYOLO Operator V1; and
...
navigate the parent tab in response to unYOLO UI actions.

## Delegated-Web V1 Transport Contract

### Session field

The HTTP field name is `unyolo-session`. Field-name comparison is
...
`unyolo.io/delegated-web/v1` session object:

```text
unyolo-session: <session.token>
```
...
- never copy it to an upstream unYOLO operator request; and
...
Every delegated-web request below uses `unyolo-session`:
...
one-time `unyolo-delegated-session` meta element or return it through the
...
`unyolo-session` to `POST <basePath>/session`. The request uses
...
authorizes the request exclusively with `unyolo-session`.
...
`unyolo-session` for protected API calls.
...
- allows `unyolo-session` and `Content-Type` in preflight responses;
...
Direct mode's browser capability also uses `unyolo-session` when calling
`/plugins/unyolo/api/v1`. A direct-mode UI can sit behind the same class of
...
## unYOLO Implementation

### UI client

Update `plugins/openclaw/ui/src/api.ts` so that:

- direct and delegated-web browser requests set only `unyolo-session`;
...
`unyolo-session`. Its capability validation, access checks, safe handles,
...
- every direct and delegated browser route sends `unyolo-session` with the
...
- the server-side Operator V1 client still sends its bearer operator
  credential and never sends `unyolo-session`;
...
- forwards `unyolo-session` unchanged to the trusted test backend;
...
```sh
pnpm --filter openclaw-unyolo check
pnpm --filter openclaw-unyolo test
...
Land unYOLO and its MLClaw consumer as one coordinated pre-release change:

1. implement and verify the unYOLO client contract;
2. implement the trusted-host adapter so the OpenClaw Gateway popover receives
   a short-lived `access: "decide"` session;
3. advance MLClaw's immutable unYOLO revision to the reviewed commit;
...
Do not publish unYOLO or MLClaw while one side still implements the replaced
bearer transport. There is no mixed-version compatibility window.

## Acceptance Criteria

- No unYOLO browser UI mode uses the standard `Authorization` field.
- `unyolo-session` is fixed, documented, tested, and used by every direct
...
are specified in
`osolmaz/mlclaw:docs/2026-07-14-unyolo-session-transport-plan.md`.
