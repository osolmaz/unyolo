...
CREATE TABLE grants (
    ...
    status TEXT NOT NULL CHECK(status IN ('pending','active','denied','failed','expired','consumed','revoked','canceled')),
    ...
    failure_reference TEXT NOT NULL DEFAULT '' CHECK(length(failure_reference) <= 256),
    failed_at TEXT,
    used_at TEXT,
    ...
    requested_max_uses INTEGER CHECK(requested_max_uses BETWEEN 1 AND 1000000),
