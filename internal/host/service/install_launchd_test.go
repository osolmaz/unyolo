...
func TestValidateLaunchdRuntimeDirectoryRejectsDuplicateAndUnsafeMode(t *testing.T) {
	...
	valid := LaunchdDirectory{Path: "/var/run/unyolo/test", Owner: "root", Group: "_broker", Mode: 0o750}
	...
	unsafeMode := LaunchdDirectory{Path: "/var/run/unyolo/other", Owner: "root", Group: "_broker", Mode: 0o777}
...
func launchdInstallFixture() LaunchdInstallPlan {
	return LaunchdInstallPlan{
		...
		ConfigDir:    "/Library/Application Support/unyolo/test", StateDir: "/Library/Application Support/unyolo/test-state",
		LaunchdDir: "/Library/LaunchDaemons", PlistName: "io.unyolo.test.plist", NoStart: true, AllowNonRoot: true,
		Files:              []ManagedFile{{Area: ManagedFileConfig, Name: "secret", Data: []byte("secret"), Mode: 0o600, Owner: ManagedFileOwnerService}},
		RuntimeDirectories: []LaunchdDirectory{{Path: "/var/run/unyolo/test", Owner: "root", Group: "_broker", Mode: 0o750}},
		Unit: LaunchdUnit{Label: "io.unyolo.test", ProgramArguments: []string{"/usr/local/bin/test", "serve"}, UserName: "_broker", GroupName: "_broker",
			Sockets: []LaunchdSocket{{Name: "agent", Path: "/var/run/unyolo/test/agent.sock", Owner: "root", Group: "broker-agent", Mode: 0o660, DirectoryMode: 0o750}}},
	}
}
