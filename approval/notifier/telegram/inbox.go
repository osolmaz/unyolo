...
func OpenInbox(ctx context.Context, path string, key []byte) (*Inbox, error) {
	if err := prepareInboxPath(path, key); err != nil {
		return nil, err
	}
...
func prepareInboxPath(path string, key []byte) error {
	if !filepath.IsAbs(path) || filepath.Clean(path) != path {
		return errors.New("telegram inbox path must be absolute and normalized")
	}
	if len(key) != 32 {
		return errors.New("telegram inbox key must be exactly 32 bytes")
	}
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		return fmt.Errorf("create telegram inbox directory: %w", err)
	}
	return ensureInboxFile(path)
}
...
func openInboxDatabase(path string) (*sql.DB, error) {
	values := url.Values{}
...
func (i *Inbox) initialize(ctx context.Context) error {
	version, err := i.formatVersion(ctx)
	if err != nil {
		return err
	}
	if err := i.createSchema(ctx); err != nil {
		return err
	}
	if version == 0 {
		return i.initializeFormatVersion(ctx)
	}
	return nil
}

func (i *Inbox) formatVersion(ctx context.Context) (int, error) {
	var version int
...
func (i *Inbox) createSchema(ctx context.Context) error {
	const schema = `
